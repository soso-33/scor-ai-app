...
if results:
    ...
    st.warning("⚠️ لا توجد نتائج لعرضها بعد. يرجى إجراء التقييم أولاً.")
...
swot_titles = {
    ...
    "ضعف": "⚠️ نقاط الضعف: تتطلب تحسين عاجل",
    "فرصة": "🚀 الفرص: يمكن استغلالها للنمو والتطوير",
    "تهديد": "⛔ التهديدات: (سيتم تحليلها لاحقًا)"
}

for key in ["قوة", "ضعف", "فرصة", "تهديد"]:
    st.markdown(f"### {swot_titles[key]}")
    if swot[key]:
        for i, item in enumerate(swot[key], 1):
            st.markdown(f"**{i}.** {item}")
    else:
        st.markdown("- لا توجد بيانات حالية.")
...
if "iot_avg" in locals():
    ...
    st.info("⚠️ لم يتم احتساب متوسط جاهزية IoT بعد.")
...
for phase in scor_phases:
    with st.expander(f"🔹 مرحلة: {phase_labels.get(phase, phase)}", expanded=True):
        ...
        if avg >= 4:
            ...
            swot["قوة"].append(
                f"{phase_labels[phase]}: أظهرت نتائج التقييم أن هذه المرحلة تعمل بكفاءة عالية (متوسط تقييم: {avg:.1f}/5). "
                f"تُظهر البيانات وجود استقرار تشغيلي واضح، مع استخدام محتمل للأدوات الرقمية في الإدارة والتحكم. "
                f"هذا المستوى من الجاهزية يُعد أساسًا قويًا يمكن تطويره ليشمل تقنيات أكثر تقدمًا مثل الذكاء التنبؤي والأتمتة."
            )
            ...
            swot["فرصة"].append(
                f"{phase_labels[phase]}: توضح النتائج أن المرحلة تعمل بمستوى مقبول (متوسط: {avg:.1f}/5)، "
                f"لكن هناك مجال واضح للتحسين. يُمكن استغلال ذلك عبر دمج أنظمة تحليل لحظي، وتحسين التكامل بين فرق العمل "
                f"والأنظمة الرقمية لدعم اتخاذ القرار المعتمد على البيانات."
            )
            ...
            colors.append("#E74C3C")
            swot["ضعف"].append(
                f"{phase_labels[phase]}: يظهر أن هذه المرحلة تعاني من ضعف ملحوظ في الجاهزية ({avg:.1f}/5)، "
                f"مما قد يعكس مشاكل في تدفق البيانات، أو ضعف في البنية الرقمية، أو غياب الرؤية التكاملية. "
