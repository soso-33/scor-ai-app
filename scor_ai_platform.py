...
page = st.sidebar.selectbox(
    "اختر الصفحة",
    ["🏠 الصفحة الرئيسية", "📊 التقييم", "🤖 التوصيات الذكية", "🏢 مقارنة الشركات"]
)
...
# بيانات افتراضية قابلة للتعديل حسب شركتك
data = pd.DataFrame({
    "المنتج": ["منتج A", "منتج B", "منتج C", "منتج D"],
    "النمو السوقي": [0.3, 0.1, 0.6, 0.05],         # معدل النمو (محور Y)
    "الحصة السوقية النسبية": [1.5, 0.4, 1.2, 0.3],  # محور X
    "الإيرادات": [300, 150, 400, 100]               # حجم الفقاعة
})
...
# خطوط التقاطع (محور 1.0 للحصة و 0.2 للنمو مثلاً)
